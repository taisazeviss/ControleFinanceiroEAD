...
-- se quiser saber so nome e email da tabela usuario
select nome_usuario, email_usuario
from tb_usuario;

-- aqui ele aparece o id mas não o nome, pq na tabela categoria não tem nome
select nome_categoria, id_usuario from tb_categoria;

-- então uso inner join (junção das tabelas)
select nome_usuario, nome_categoria, email_usuario
	from tb_categoria
inner join tb_usuario
	on tb_categoria.id_usuario = tb_usuario.id_tusuario;
